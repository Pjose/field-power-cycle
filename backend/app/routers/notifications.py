...
def toggle_rule(rule_id: str, body: schemas.NotificationRuleToggleIn, db: Session = Depends(get_db)):
    rule = db.get(models.NotificationRule, rule_id)
    if not rule:
        raise HTTPException(404, "rule not found")
    rule.enabled = body.enabled
    log_event(db, "notification_rule_toggled", "notification_rule", rule.id,
              actor="dispatcher", payload={"enabled": body.enabled})
    db.commit()
...
def notification_feed(limit: int = 50, db: Session = Depends(get_db)):
